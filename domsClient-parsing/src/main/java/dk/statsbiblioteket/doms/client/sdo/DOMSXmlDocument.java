...
public class DOMSXmlDocument {
    ...
    public void generate(DOMSDataStreamCompositeModelTypeModelExtensionSchema compositeSchema) throws IOException,
                                                                                                      MyXMLWriteException,
                                                                                                      MyXMLParseException, ServerOperationFailed {
        sdoContext = SDOUtil.createHelperContext(true);
        InputStream is = null;

        try
        {
            if (compositeSchema.getType().equals("datastream")){

                is = repository().getDatastreamDissemination(compositeSchema.getObject(),compositeSchema.getValue());
                ...
                DOMSXMLDocumentOld doc = new DOMSXMLDocumentOld();
                if (doc.Load(is))
                {
                ...
                setSdoTypes(defineTypes(sdoContext, repository(), compositeSchema));
                if (getSdoTypes()!=null)
                {
    ...
    public void load(DOMSDataStreamCompositeModelTypeModelExtensionSchema compositeSchema, InputStream is) throws IOException, DOMSIllegalStateException
    {
        ...
        try
        {
            ...
            Property rootProperty = getRootProperty(sdoTypes, targetNamespace, sdoContext.getXSDHelper());

            if (rootProperty==null){
                throw new DOMSIllegalStateException("Could not get a valid SDO root DataObject for Input Stream: '" + is +	"'.");
            }
    ...
    private List<Type> defineTypes(HelperContext context, RepositoryBean repository, DOMSDataStreamCompositeModelTypeModelExtensionSchema compositeSchema)
            throws RemoteException, IOException, ServerOperationFailed {
        ...
        {
            if (compositeSchema.getType().equals("datastream")){
                String fedoraUrl = compositeSchema.getObject() + "/" + compositeSchema.getValue();

                is = repository.getDatastreamDissemination(compositeSchema.getObject(),compositeSchema.getValue());

                //Define the SDO types based on the XML Schema
                types = context.getXSDHelper().define(is, fedoraUrl);
            }
